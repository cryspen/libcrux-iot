...
theorem keccak.keccak_loop0_spec
  ...
  · intro acc k h_ge h_le_k hinv
    obtain ⟨h_acc_i, h_fold_acc⟩ := of_pure_prop_holds hinv
    -- Body: keccak.keccak_loop0.body RATE data { start := k, end := n } acc.
    ...
    · rintro ⟨hge, hiter1_eq⟩
      show ⦃⌜True⌝⦄ (Aeneas.Std.Result.ok (ControlFlow.done acc) : Result _) ⦃_⦄
      have hk_eq : k.val = n.val := Nat.le_antisymm h_le_k hge
      apply Sponge.triple_of_ok_abs (v := acc) rfl
      refine pure_prop_holds ⟨h_acc_i, ?_⟩
      -- Goal: absorb_fold s data RATE n.val = .ok (lift acc). Transfer via hk_eq.
      have hk_subst : k.val = n.val := hk_eq
      rw [← hk_subst]
      exact h_fold_acc
    · rintro ⟨hi_eq, hk_lt, hiter1_end, hiter1_start⟩
      ...
      have hk1_RATE_le : (k.val + 1) * RATE.val ≤ n.val * RATE.val := by
        have := Nat.mul_le_mul_right RATE.val hk1_le_n
        simpa using this
      have hk_RATE_data : k.val * RATE.val + RATE.val ≤ data.val.length := by
        have h2 : (k.val + 1) * RATE.val = k.val * RATE.val + RATE.val := by ring
        omega
      have hk_RATE_max : k.val * RATE.val + RATE.val ≤ Std.Usize.max := by
        have h2 : (k.val + 1) * RATE.val = k.val * RATE.val + RATE.val := by ring
        omega
      -- The body uses `i * RATE` (Usize mul). Then absorb_block. Use mvcgen.
      mvcgen
      all_goals (try scalar_tac)
      -- Remaining VC: invariant preservation (the absorb_block side conditions
      -- were all discharged by `scalar_tac` since hypotheses are in scope).
      -- The invariant preservation VC.
      expose_names
      obtain ⟨h_r1_i, h_r1_spec⟩ := h_1
      refine ⟨hk_lt, hiter1_end, hiter1_start, ?_⟩
      apply pure_prop_holds
      refine ⟨h_r1_i, ?_⟩
      -- Goal: absorb_fold s data RATE iter1.start.val = .ok (lift r_1).
      -- iter1.start.val = k.val + 1 (via hiter1_start).
      rw [hiter1_start]
      -- Unfold fold at k+1 and use Nat.fold_succ.
      unfold absorb_fold
      rw [Nat.fold_succ]
      ...
      rw [h_inner]
      simp only [bind_tc_ok]
      -- Now the goal: sponge.absorb_block (lift acc) ⟨data.slice (k*RATE) ((k+1)*RATE), _⟩ RATE = .ok (lift r_1).
      -- We have: h_r1_spec : sponge.absorb_block (lift acc) (block_of_blocks data r RATE _) RATE = .ok (lift r_1).
      -- block_of_blocks data r RATE _ has .val = data.val.slice r.val (r.val + RATE.val).
      -- And h : r.val = k.val * RATE.val, so r.val + RATE.val = (k+1)*RATE.val.
      -- The two slices are equal: data.val.slice r.val (r.val + RATE.val)
      -- vs data.val.slice (k*RATE) ((k+1)*RATE). The block_of_blocks
      -- helper (private to AbsorbBlock.lean) is what mvcgen expanded;
      -- we use `r.val = k.val * RATE.val` and `(k+1)*RATE = k*RATE + RATE`.
      have h_slice_eq :
          data.val.slice (k.val * RATE.val) ((k.val + 1) * RATE.val)
            = data.val.slice r.val (r.val + RATE.val) := by
        rw [h]
        congr 1
        ring
      convert h_r1_spec using 2
      apply Subtype.ext
      show data.val.slice _ _ = _
      rw [h_slice_eq]
      rfl
