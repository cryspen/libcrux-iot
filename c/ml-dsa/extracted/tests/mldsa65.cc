...
TEST(MlDsa65TestPortable, ConsistencyTest)
{
    ...
    bytes signing_key(LIBCRUX_IOT_ML_DSA_ML_DSA_GENERIC_ML_DSA_65_SIGNING_KEY_SIZE);
    bytes verification_key(LIBCRUX_IOT_ML_DSA_ML_DSA_GENERIC_ML_DSA_65_VERIFICATION_KEY_SIZE);
    ...
    bytes signature(LIBCRUX_IOT_ML_DSA_ML_DSA_GENERIC_ML_DSA_65_SIGNATURE_SIZE);
    ...
    libcrux_iot_ml_dsa_ml_dsa_generic_ml_dsa_65_MLDSA65VerificationKey verification_key_struct;
    memcpy(verification_key_struct.value, verification_key.data(), verification_key.size());
    libcrux_iot_ml_dsa_ml_dsa_generic_ml_dsa_65_MLDSA65Signature signature_struct;
...
TEST(MlDsa65TestAvx2, ConsistencyTest)
{
    ...
    auto key_pair = libcrux_iot_ml_dsa_ml_dsa_65_avx2_generate_key_pair(randomness);
    ...
    auto msg_slice = mk_slice(&msg, 79);
    auto context_slice = mk_slice(&context, 3);
    auto signature_result = libcrux_iot_ml_dsa_ml_dsa_65_avx2_sign(
    ...
    // Verify
    auto result = libcrux_iot_ml_dsa_ml_dsa_65_avx2_verify(
