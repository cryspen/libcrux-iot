...
fn serialize_when_gamma1_is_2_pow_17(simd_unit: &Coefficients, serialized: &mut [u8]) {
    ...
    cloop! {
        for (i, coefficients) in simd_unit.values.chunks_exact(4).enumerate() {
            ...
            let coefficient0 = GAMMA1 - coefficients[0].declassify();
            let coefficient1 = GAMMA1 - coefficients[1].declassify();
            let coefficient2 = GAMMA1 - coefficients[2].declassify();
            let coefficient3 = GAMMA1 - coefficients[3].declassify();
...
fn deserialize_when_gamma1_is_2_pow_17(serialized: &[U8], simd_unit: &mut Coefficients) {
    ...
    cloop! {
        for (i, bytes) in serialized.chunks_exact(9).enumerate() {
            ...
            simd_unit.values[4 * i] = GAMMA1.classify() - coefficient0;
            simd_unit.values[4 * i + 1] = GAMMA1.classify() - coefficient1;
            simd_unit.values[4 * i + 2] = GAMMA1.classify() - coefficient2;
            simd_unit.values[4 * i + 3] = GAMMA1.classify() - coefficient3;
        }
    }
...
fn deserialize_when_gamma1_is_2_pow_19(serialized: &[U8], simd_unit: &mut Coefficients) {
    ...
    cloop! {
        for (i, bytes) in serialized.chunks_exact(5).enumerate() {
            ...
            simd_unit.values[2 * i] = GAMMA1.classify() - coefficient0;
            simd_unit.values[2 * i + 1] = GAMMA1.classify() - coefficient1;
        }
    }
