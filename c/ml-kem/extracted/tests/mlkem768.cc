...
TEST(MlKem768TestPortable, ConsistencyTest)
{
    ...
    auto key_pair = libcrux_iot_ml_kem_mlkem768_portable_generate_key_pair(randomness);
    ...
    auto ctxt = libcrux_iot_ml_kem_mlkem768_portable_encapsulate(&key_pair.pk, randomness);

    // cout << "ctxt: " << bytes_to_hex(bytes(ctxt.fst.value, ctxt.fst.value + 16U)) << endl;

    uint8_t sharedSecret2[LIBCRUX_IOT_ML_KEM_CONSTANTS_SHARED_SECRET_SIZE];
    libcrux_iot_ml_kem_mlkem768_portable_decapsulate(&key_pair.sk, &ctxt.fst, sharedSecret2);
...
TEST(MlKem768TestPortableUnpacked, ConsistencyTest)
{
    ...
    auto key_pair = libcrux_iot_ml_kem_mlkem768_portable_generate_key_pair_unpacked(randomness);

    uint8_t randomness2[32];
    generate_random(randomness2, 32);
    auto ctxt = libcrux_iot_ml_kem_mlkem768_portable_encapsulate_unpacked(&key_pair.public_key, randomness2);

    uint8_t sharedSecret2[LIBCRUX_IOT_ML_KEM_CONSTANTS_SHARED_SECRET_SIZE];
    libcrux_iot_ml_kem_mlkem768_portable_decapsulate_unpacked(&key_pair, &ctxt.fst, sharedSecret2);
...
TEST(Kyber768TestPortable, ModifiedCiphertextTest)
{
    ...
    uint8_t *implicitRejectionSharedSecret =
        compute_implicit_rejection_shared_secret(
            ...
            LIBCRUX_IOT_ML_KEM_MLKEM768_CPA_PKE_CIPHERTEXT_SIZE,
            key_pair.sk.value,
            LIBCRUX_IOT_ML_KEM_MLKEM768_SECRET_KEY_SIZE);
...
TEST(Kyber768TestPortable, ModifiedSecretKeyTest)
{
    ...
    uint8_t sharedSecret2[LIBCRUX_IOT_ML_KEM_CONSTANTS_SHARED_SECRET_SIZE];
    modify_secret_key(
        key_pair.sk.value, LIBCRUX_IOT_ML_KEM_MLKEM768_SECRET_KEY_SIZE, false);
    libcrux_iot_ml_kem_mlkem768_portable_decapsulate(&key_pair.sk, &ctxt.fst, sharedSecret2);
    ...
    modify_secret_key(
        ctxt.snd, LIBCRUX_IOT_ML_KEM_MLKEM768_SECRET_KEY_SIZE, true);
    libcrux_iot_ml_kem_mlkem768_portable_decapsulate(&key_pair.sk, &ctxt.fst, sharedSecret2);
...
TEST(MlKem768TestPortable, NISTKnownAnswerTest)
{
    ...
    {
        auto key_pair =
            libcrux_iot_ml_kem_mlkem768_portable_generate_key_pair(kat.key_generation_seed.data());
        ...
        uint8_t sk_hash[32];
        sha256(
            mk_slice(key_pair.sk.value, LIBCRUX_IOT_ML_KEM_MLKEM768_SECRET_KEY_SIZE), sk_hash);
        EXPECT_EQ(0, memcmp(sk_hash, kat.sha3_256_hash_of_secret_key.data(), 32));

        auto ctxt = libcrux_iot_ml_kem_mlkem768_portable_encapsulate(
...
TEST(MlKem768TestPortableUnpacked, NISTKnownAnswerTest)
{
    ...
    {
        auto key_pair =
            libcrux_iot_ml_kem_mlkem768_portable_generate_key_pair_unpacked(kat.key_generation_seed.data());

        // We can't check the keys because we don't really have them.

        auto ctxt = libcrux_iot_ml_kem_mlkem768_portable_encapsulate_unpacked(&key_pair.public_key, kat.encapsulation_seed.data());
...
TEST(MlKem768TestNeon, ConsistencyTest)
{
    ...
    for (int i = 0; i < 64; i++)
        randomness[i] = 13;
    auto key_pair = libcrux_iot_ml_kem_mlkem768_neon_generate_key_pair(randomness);
    auto ctxt = libcrux_iot_ml_kem_mlkem768_neon_encapsulate(&key_pair.pk, randomness);

    uint8_t sharedSecret2[LIBCRUX_IOT_ML_KEM_CONSTANTS_SHARED_SECRET_SIZE];
    libcrux_iot_ml_kem_mlkem768_neon_decapsulate(&key_pair.sk, &ctxt.fst, sharedSecret2);
...
TEST(MlKem768TestNeon, NISTKnownAnswerTest)
{
    ...
    for (auto kat : kats)
    {
        auto key_pair = libcrux_iot_ml_kem_mlkem768_neon_generate_key_pair(kat.key_generation_seed.data());
        ...
        auto ctxt = libcrux_iot_ml_kem_mlkem768_neon_encapsulate(
            &key_pair.pk, kat.encapsulation_seed.data());
        uint8_t ct_hash[32];
        sha256(
            mk_slice(ctxt.fst.value,
