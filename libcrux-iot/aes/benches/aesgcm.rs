                ...
                group.bench_with_input(
                    ...
                    |b, payload_size| {
                        b.iter_batched(
                            ...
                            |(key, nonce, aad, payload)| {
                                let mut ciphertext = vec![0; *payload_size];
                                use libcrux_iot_aes::$fun::neon::{$neon, Key, Nonce, Tag};

                                let k: Key<$neon> = key.into();
                                let nonce: Nonce<$neon> = nonce.into();
                                let mut tag: Tag<$neon> = [0; libcrux_iot_aes::TAG_LEN].into();
                ...
                group.bench_with_input(
                    ...
                    |b, payload_size| {
                        b.iter_batched(
                            ...
                            |(key, nonce, aad, payload)| {
                                let mut ciphertext = vec![0; *payload_size];
                                use libcrux_iot_aes::$fun::x64::{$intel, Key, Nonce, Tag};

                                let k: Key<$intel> = key.into();
                                let nonce: Nonce<$intel> = nonce.into();
                                let mut tag: Tag<$intel> = [0; libcrux_iot_aes::TAG_LEN].into();
