...
abbrev enumCENext (it : EnumCE) :
    Result ((core_models.option.Option (Std.Usize × Slice Std.U8)) × EnumCE) :=
...
theorem enumerate_chunks_next_cont
    ...
    ∃ (chunk drop : Slice Std.U8) (cnt' : Std.Usize),
      enumCENext { iter := { cs := cs, elements := rest }, count := cnt }
        = .ok (core_models.option.Option.Some (cnt, chunk),
...
private theorem compute_ring_element_v_loop_step_lemma_fc
  ...
  · -- `Some (cnt, chunk)` branch.
    ...
    have h_idx_r : Aeneas.Std.Slice.index_usize r_as_ntt cnt = .ok t_r := by
      rw [ht_r_def, ← h_cnt]
      exact libcrux_iot_ml_kem.Util.slice_index_usize_ok_eq r_as_ntt cnt
    ...
    have h_idx_cache : Aeneas.Std.Slice.index_usize cache cnt = .ok t_cache := by
      ...
      exact libcrux_iot_ml_kem.Util.slice_index_usize_ok_eq cache cnt
...
-- Axiom hygiene: reports {propext, Classical.choice, Quot.sound,
-- deserialize_to_reduced_ring_element_fc, sorryAx}. The A2 deserialize axiom is
-- the SANCTIONED L7.3 boundary; the `sorryAx` is STRUCTURALLY INHERITED via the
-- POST's `lift_t_as_ntt_from_public_key` = `Spec.t_as_ntt_from_public_key_pure`
-- stub (FCTargets:271) — NOT a proof hole (this file has zero `sorry` tactics).
-- Same precedent as L7.2 `compute_vector_u_loop1_loop0_fc`.
#print axioms compute_ring_element_v_loop_fc
...
-- Axiom hygiene: the bridge carries the SANCTIONED `sorryAx` (structurally
-- inherited via `lift_t_as_ntt_from_public_key` → `Spec.t_as_ntt_from_public_key_pure`
-- stub) + the A2 deserialize axiom. Expect {propext, Classical.choice,
-- Quot.sound, deserialize_to_reduced_ring_element_fc, sorryAx}.
#print axioms compute_ring_element_v_acc_bridge
