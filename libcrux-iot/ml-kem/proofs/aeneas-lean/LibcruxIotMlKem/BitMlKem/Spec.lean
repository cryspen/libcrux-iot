/-
  # `BitMlKem/Spec.lean` — pure-Lean intermediate spec for ML-KEM.

  Mirrors `HacspecMlKem`'s `ntt`, `multiply_ntts`, `compress`, …,
  but works on `MontPoly = Vector (ZMod 3329) 256` so that
  `ring`/`field_simp` close the algebraic commute lemmas in
  Layer M.4 without poking raw `% 3329` arithmetic.
  ...
  *and* the impl-side `LibcruxIotMlKem.Extraction.Funs` transitively
  ...
  per Plan.lean §X.2 / K.1). No Triples here (those land in M.4 +
  L0+ upgrades per §X.3).
-/
...
/-! ## §B.3 — the 34 `bit_<op>` defs.
    ...
    - "complex" ops (NTT family, INTT, multiply_ntts, compress,
      ...
      `TODO(M.4)` so the upgrade pass can find them mechanically.
-/
...
/-! ### #8-12 — NTT layer family (STUBBED, real bodies in M.4) -/
...
def bit_ntt_layer_1 (p : MontPoly) (_zetas : Vector (ZMod 3329) 64) :
    MontPoly := p
  -- TODO(M.4): replace with real butterfly-pass body.
...
def bit_ntt_layer_4_to_7 (p : MontPoly) (_layer : Nat) : MontPoly := p
  -- TODO(M.4): replace with real layer-parametric butterfly body.
...
def bit_ntt (p : MontPoly) : MontPoly := p
  -- TODO(M.4): replace with `bit_ntt_layer_1 ∘ … ∘ bit_ntt_layer_4_to_7 7`.
...
def bit_invert_ntt_layer_1 (p : MontPoly) (_zetas : Vector (ZMod 3329) 64) :
    MontPoly := p
  -- TODO(M.4): replace with real inverse-butterfly body.
...
def bit_invert_ntt_layer_4_to_7 (p : MontPoly) (_layer : Nat) : MontPoly := p
  -- TODO(M.4): replace with real layer-parametric inverse body.
...
def bit_invert_ntt_montgomery (p : MontPoly) : MontPoly := p
  -- TODO(M.4): replace with real INTT-without-finalize body.
...
def bit_ntt_multiply_n (p q : MontPoly) (_zetas : Vector (ZMod 3329) 64) :
    MontPoly := p + q -- harmless placeholder; replaced in M.4
  -- TODO(M.4): replace with real per-pair base-case multiply body.
...
def bit_multiply_ntts (p q : MontPoly) : MontPoly := p + q
  -- TODO(M.4): replace with real body.
...
def bit_base_case_multiply_even
  ...
  -- TODO(M.4): replace with `a0*b0 + zeta*a1*b1`.
...
def bit_base_case_multiply_odd
  ...
  -- TODO(M.4): replace with `a0*b1 + a1*b0`.
...
def bit_butterfly (_zeta a b : ZMod 3329) :
  ...
  -- TODO(M.4): replace with `(a + zeta*b, a - zeta*b)`.
...
def bit_inv_butterfly (_zeta a b : ZMod 3329) :
    ZMod 3329 × ZMod 3329 := (a, b)
  -- TODO(M.4): replace with the real inv-butterfly body.
...
def bit_compress (_p : MontPoly) (_d : Nat) : Vector (ZMod 3329) 256 :=
  ...
  -- TODO(M.4): replace with real per-lane compression body; return type
...
def bit_decompress (_c : Vector (ZMod 3329) 256) (_d : Nat) : MontPoly :=
  Vector.replicate 256 (0 : ZMod 3329)
  -- TODO(M.4): replace with real per-lane decompression body.
...
def bit_byte_encode (_p : MontPoly) (_d : Nat) : List Std.U8 := []
  -- TODO(M.4): real signature is `Vector Std.U8 (32 * d)`; replace
...
def bit_byte_decode (_bytes : List Std.U8) (_d : Nat) : MontPoly :=
  Vector.replicate 256 (0 : ZMod 3329)
  -- TODO(M.4): real signature is `Vector Std.U8 (32 * d) → MontPoly`;
...
def bit_ntt_layer_int_vec_step
  ...
  -- TODO(M.4): replace with the per-step butterfly body.
...
def bit_accumulating_ntt_multiply
  ...
  -- TODO(M.4): replace with the per-pair base-case accumulator body.
