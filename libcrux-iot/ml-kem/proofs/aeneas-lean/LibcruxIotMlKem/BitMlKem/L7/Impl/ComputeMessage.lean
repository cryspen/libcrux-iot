...
def step_post {K : Std.Usize}
    ...
    (r : ControlFlow
      ((core_models.ops.range.Range Std.Usize) × Acc) Acc) :
    Prop :=
  match r with
  | .cont (iter', acc') =>
  ...
  · -- `Some k` branch.
    ...
    have h_body :
        ...
        = .ok (ControlFlow.cont (({ start := s_iter, «end» := K }
                        : core_models.ops.range.Range Std.Usize), acc1)) := by
      unfold libcrux_iot_ml_kem.matrix.compute_message_loop.body
      conv_lhs =>
        rw [show
      ...
      show ((do
              ...
              .ok (ControlFlow.cont (({ start := s_iter, «end» := K }
                          : core_models.ops.range.Range Std.Usize), accumulator1)))
            : Result _) = _
      rw [h_idx_secret]
      simp only [Aeneas.Std.bind_tc_ok]
      rw [h_idx_u]
    ...
    show L7_4_FC.step_post secret_as_ntt u_as_ntt acc_init k
      (.cont (({ start := s_iter, «end» := K }
                : core_models.ops.range.Range Std.Usize), acc1))
...
/-! ### Phase 1 — `multiply_vectors` loop reduction (mirror of
...
/-- **Phase 1.** `multiply_vectors (lift_vec s) (lift_vec u)` reduces to the
...
/-! ### Phase 1b — re-derived Helper 1 (`multiply_ntts_lane_eq_canonical_factor`).
...
/-! ### Phase 2 — local canonical-determination helper + per-lane bridge. -/
...
/-- **A — acc-bridge / "crux" (CLOSED).**
    ...
    Proof: Phase 1 reduces `multiply_vectors` to the per-lane `add_pure`-foldl
...
theorem compute_message_acc_bridge {K : Std.Usize}
  ...
  -- Phase 1: reduce the hacspec loop to the pure `add_pure`-foldl array.
