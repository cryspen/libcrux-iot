/-
  ...
  `LibcruxIotMlKem/Util/LoopSpecs.lean`, with the namespace rewritten
  from `libcrux_iot_ml_kem.Util` → `libcrux_iot_ml_dsa.Util`. The file is
  scalar-generic over `Usize` / `I32`, matching the ML-DSA NTT loop drivers
  (coefficients are `Std.I32`, range iteration is over `Usize`).
...
namespace libcrux_iot_ml_dsa.Util.LoopSpecs
...
-- (`libcrux_iot_ml_dsa.Util.SliceSpecs.result_eq_of_triple` is in scope via `Util.SliceSpecs`.)
...
end libcrux_iot_ml_dsa.Util.LoopSpecs
