...
TEST(MlKem1024TestPortable, ConsistencyTest) {
  ...
  auto key_pair = libcrux_iot_ml_kem_mlkem1024_portable_generate_key_pair(r);

  Eurydice_arr_600 r2 = {0};
  memset(r2.data, 0x15, 32);
  auto ctxt = libcrux_iot_ml_kem_mlkem1024_portable_encapsulate(&key_pair.pk, r2);

  auto sharedSecret2 =
      libcrux_iot_ml_kem_mlkem1024_portable_decapsulate(&key_pair.sk, &ctxt.fst);
...
TEST(Kyber1024TestPortable, ModifiedCiphertextTest) {
  ...
  auto key_pair =
      libcrux_iot_ml_kem_mlkem1024_portable_generate_key_pair(randomness1);
  ...
  uint8_t *implicitRejectionSharedSecret =
      compute_implicit_rejection_shared_secret(
          ctxt.fst.data, LIBCRUX_IOT_ML_KEM_MLKEM1024_CPA_PKE_CIPHERTEXT_SIZE,
          key_pair.sk.data, LIBCRUX_IOT_ML_KEM_MLKEM1024_SECRET_KEY_SIZE);
...
TEST(Kyber1024TestPortable, ModifiedSecretKeyTest) {
  ...
  modify_secret_key(key_pair.sk.data, LIBCRUX_IOT_ML_KEM_MLKEM1024_SECRET_KEY_SIZE,
                    false);
  auto sharedSecret2 =
      libcrux_iot_ml_kem_mlkem1024_portable_decapsulate(&key_pair.sk, &ctxt.fst);
  ...
  modify_secret_key(ctxt.snd.data, LIBCRUX_IOT_ML_KEM_MLKEM1024_SECRET_KEY_SIZE,
                    true);
  auto sharedSecret3 =
      libcrux_iot_ml_kem_mlkem1024_portable_decapsulate(&key_pair.sk, &ctxt.fst);
...
TEST(MlKem1024TestPortable, NISTKnownAnswerTest) {
  ...
  for (auto kat : kats) {
    ...
    auto key_pair =
        libcrux_iot_ml_kem_mlkem1024_portable_generate_key_pair(keygen_rand);

    auto pk_hash = sha256(mk_borrow_slice_u8(
        key_pair.pk.data, LIBCRUX_IOT_ML_KEM_MLKEM1024_CPA_PKE_PUBLIC_KEY_SIZE));
    EXPECT_EQ(0,
    ...
    auto ctxt = libcrux_iot_ml_kem_mlkem1024_portable_encapsulate(&key_pair.pk,
                                                              encaps_rand);
...
TEST(MlKem1024TestPortableUnpacked, NISTKnownAnswerTest) {
  ...
  for (auto kat : kats) {
    auto key_pair =
        libcrux_iot_ml_kem_mlkem1024_portable_generate_key_pair_unpacked(
            kat.key_generation_seed.data());

    // We can't check the keys because we don't really have them.

    auto ctxt = libcrux_iot_ml_kem_mlkem1024_portable_encapsulate_unpacked(
    ...
    libcrux_iot_ml_kem_mlkem1024_portable_decapsulate_unpacked(&key_pair, &ctxt.fst,
                                                           sharedSecret2);
...
TEST(MlKem1024TestNeon, ConsistencyTest) {
  ...
  auto key_pair = libcrux_iot_ml_kem_mlkem1024_neon_generate_key_pair(randomness);
  auto ctxt =
      libcrux_iot_ml_kem_mlkem1024_neon_encapsulate(&key_pair.pk, randomness);

  uint8_t sharedSecret2[LIBCRUX_IOT_ML_KEM_CONSTANTS_SHARED_SECRET_SIZE];
  libcrux_iot_ml_kem_mlkem1024_neon_decapsulate(&key_pair.sk, &ctxt.fst,
                                            sharedSecret2);
...
TEST(MlKem1024TestNeon, NISTKnownAnswerTest) {
  ...
  for (auto kat : kats) {
    auto key_pair = libcrux_iot_ml_kem_mlkem1024_neon_generate_key_pair(
        kat.key_generation_seed.data());
    ...
    libcrux_sha3_sha256(
        mk_slice(key_pair.sk.value, LIBCRUX_IOT_ML_KEM_MLKEM1024_SECRET_KEY_SIZE),
        sk_hash);
    EXPECT_EQ(0, memcmp(sk_hash, kat.sha3_256_hash_of_secret_key.data(), 32));

    auto ctxt = libcrux_iot_ml_kem_mlkem1024_neon_encapsulate(
