...
theorem spec_theta_unrolled_eq (state : RustArray u64 25) :
  ...
  have hc := hacspec_sha3.createi_ofFn (n := 5) True
    (fun x => do
      ...
      pure (v0123 ^^^ v4))
    (fun i => by native_decide) -- 
placeholder_no_use simp [Std.Do.Triple, Std.Do.WP.wp, Std.Do.PredTrans.apply, Std.Do.PredTrans.pushExcept, Std.Do.PredTrans.pure,
        Std.Do.PredTrans.trans, ExceptT.run, Id.run, pure, bind, RustM.bind,
  ...
  have hd := hacspec_sha3.createi_ofFn (n := 5) True
    ...
    (fun i => by fin_cases i <;>
      simp [Std.Do.Triple, Std.Do.WP.wp, Std.Do.PredTrans.apply, Std.Do.PredTrans.pushExcept, Std.Do.PredTrans.pure,
        Std.Do.PredTrans.trans, ExceptT.run, Id.run, pure, bind, RustM.bind,
