/-
  # L7.2 `compute_vector_u` — per-CONTRACT (per-seam) numerical validation.

  Mirrors `ComputeMessageSeamValidation.lean` (L7.4) and
  `ComputeRingElementVSeamValidation.lean` (L7.3). This file validates each
  INTERNAL seam of one PER-ROW slice of `matrix.compute_vector_u` by running
  impl and hacspec spec STEP-BY-STEP (all per-step functions are computable)
  and pinning the exact per-lane Montgomery R-factor at each boundary. This is
  the per-contract property testing that end-to-end testing is structurally
  blind to. PROPERTY TESTING via `#guard` (computable proxies in `ZMod 3329`),
  NOT a proof. NO sorry.
  ...
  Validated factors (spec_lane = factor · impl_lane, ALL 256 lanes, K=2):
    SEAM 1  reducing_from_i32_array out (lift result1)  vs  multiply_vectors
            → factor 2285 = R = 2¹⁶ mod q.        [re-confirms L7.4/L7.3 SEAM 1,
                                                    now at the per-row level]
    SEAM 2  invert_ntt_montgomery out (lift result2)    vs  ntt_inverse
            → factor 512 = 1441·169 mod q.        [from L7.4/L7.3 SEAM 2]
    TAIL    add_error_reduce out (lift result_i) vs
            add_polynomials(scaleZ 512 (lift result2), error_1[i])
            → factor 1 (THE new seam — the FC tail bridge D'', SINGLE-add).
  ...
  result[i] = inv + error_1[i] = 512·result2 + error_1[i].  ⟹ impl result_i ==
  hacspec result[i] at matching inputs, factor 1.  This differs from L7.3's
  `add_message_error_reduce` only by having ONE add (error only) instead of two
  (error + message); the tail bridge collapses to a single `add_polynomials`.
  ...
  TRANSPOSE / COLUMN-MAJOR RECONCILIATION (axiom-handled, EXCLUDED from PBT):
    ...
    (column-major) is the `sample_matrix_entry_fc` AXIOM at
    `BitMlKem/L7/Axioms.lean:115`. `lift_matrix_from_seed` is NONCOMPUTABLE
    (`Spec.sample_matrix_A_pure` is a `sorry` stub at FCTargets:219), so the
    seed→matrix sampling / transpose convention CANNOT be tested numerically;
    it is excluded here and discharged by that axiom in the proof.

  Consequence for the proof contracts:
    L7.2 per-row FC tail bridge D'' :
      add_polynomials(scaleZ 512 (lift result2), lift error_1[i]) = lift result_i,
    with the product→invert seams (A/B/C) reused VERBATIM from L7.4/L7.3.

  Inner-product construction path used: the L7.4/L7.3 fallback — the impl
  accumulator is built via `matrix.compute_message_loop mRow r accArr`
  (secret:=the chosen matrix row = (m0,m1), u:=r). `multiply_vectors` is
  structurally symmetric in its two vector arguments, so this reproduces the
  per-row inner product `Σ_j aT[i][j]·r[j]` and validates the 2285 factor
  without sampling a matrix by hand.

  TAIL PRE-bounds (`add_error_reduce_fc` PRE, FCTargets:15642):
  ...
  Not imported by the root library. Author: orchestrator (L7.2 per-contract, 2026-05-29).
-/
