...
-- Minimal definitions needed (duplicated for clean context)
def spread_to_even' (x : BitVec 32) : BitVec 64 :=
  ...
  x
def lift_lane_bv' (z0 z1 : BitVec 32) : BitVec 64 := spread_to_even' z0 ||| (spread_to_even' z1 <<< 1)
def lift_lane' (l : Lane2U32) : u64 := UInt64.ofBitVec (lift_lane_bv' l._0.toVec[0].toBitVec l._0.toVec[1].toBitVec)
def lift' (s : KeccakState) : RustArray u64 25 := RustArray.ofVec (.ofFn fun i => lift_lane' s.st.toVec[i])
def impl_perm' (i : Fin 25) : Fin 25 := let x := i.val / 5; let z := i.val % 5; ⟨5 * x + (3 * z + 2 * x) % 5, by omega⟩
def impl_perm2' (i : Fin 25) : Fin 25 := impl_perm' (impl_perm' i)
def impl_perm3' (i : Fin 25) : Fin 25 := impl_perm' (impl_perm' (impl_perm' i))
def lift_perm' (s : KeccakState) (p : Fin 25 → Fin 25) : RustArray u64 25 := RustArray.ofVec (.ofFn fun i => lift_lane' s.st.toVec[(p i).val])
...
theorem chi_lane_lift' (bx0_z0 bx0_z1 bx1_z0 bx1_z1 bx2_z0 bx2_z1 : BitVec 32) : lift_lane_bv' (bx0_z0 ^^^ ((~~~bx1_z0) &&& bx2_z0)) (bx0_z1 ^^^ ((~~~bx1_z1) &&& bx2_z1)) = lift_lane_bv' bx0_z0 bx0_z1 ^^^ ((~~~(lift_lane_bv' bx1_z0 bx1_z1)) &&& lift_lane_bv' bx2_z0 bx2_z1) := by simp only [lift_lane_bv_xor', lift_lane_bv_not', lift_lane_bv_and']

-- Round definitions
def spec_round' (state : RustArray u64 25) (round : usize) : RustM (RustArray u64 25) := do
  let s ← hacspec_sha3.keccak_f.theta state; let s ← hacspec_sha3.keccak_f.rho s
  let s ← hacspec_sha3.keccak_f.pi s; let s ← hacspec_sha3.keccak_f.chi s
