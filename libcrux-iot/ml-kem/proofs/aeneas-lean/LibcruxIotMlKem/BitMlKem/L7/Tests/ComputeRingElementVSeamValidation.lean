/-
  # L7.3 `compute_ring_element_v` — per-CONTRACT (per-seam) numerical validation.

  Mirrors `ComputeMessageSeamValidation.lean` (L7.4). This file validates each
  INTERNAL seam of `matrix.compute_ring_element_v` by running impl and hacspec
  spec STEP-BY-STEP (all per-step functions are computable) and pinning the
  exact per-lane Montgomery R-factor at each boundary. This is the per-contract
  property testing that end-to-end testing is structurally blind to. PROPERTY
  TESTING via `#guard` (computable proxies in `ZMod 3329`), NOT a proof. NO sorry.
  ...
  = inv + e2 + msg = 512·result2 + e2 + msg.  ⟹ impl result3 == hacspec result
  at matching inputs, factor 1.

  Consequence for the proof contracts:
    L7.3 FC tail bridge D' :
      add_polys(add_polys(scaleZ 512 (lift result2), e2), msg) = lift result3,
    with the product→invert seams (A/B/C) reused verbatim from L7.4.

  Inner-product construction path used: the L7.4 fallback — the impl accumulator
  is built via `matrix.compute_message_loop t r accArr` (secret:=t, u:=r). Its
  per-lane semantics are exactly `accumulating_ntt_multiply` over the K=2 vector,
  identical to what `compute_ring_element_v_loop` produces via
  `accumulating_ntt_multiply_use_cache` for a correctly-filled cache; this
  validates the 2285 factor without constructing a cache by hand.
  ...
  Not imported by the root library. Author: orchestrator (L7.3 per-contract, 2026-05-29).
-/
