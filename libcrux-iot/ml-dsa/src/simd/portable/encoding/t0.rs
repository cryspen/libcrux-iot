use libcrux_secrets::{CastOps as _, Classify as _, IntOps as _, I32, U8};
...
pub fn serialize(simd_unit: &Coefficients, serialized: &mut [U8]) {
    ...
    serialized[0] = coefficient0.as_u8();

    serialized[1] = (coefficient0 >> 8).as_u8();
    serialized[1] |= (coefficient1 << 5).as_u8();

    serialized[2] = (coefficient1 >> 3).as_u8();

    serialized[3] = (coefficient1 >> 11).as_u8();
    serialized[3] |= (coefficient2 << 2).as_u8();

    serialized[4] = (coefficient2 >> 6).as_u8();
    serialized[4] |= (coefficient3 << 7).as_u8();

    serialized[5] = (coefficient3 >> 1).as_u8();

    serialized[6] = (coefficient3 >> 9).as_u8();
    serialized[6] |= (coefficient4 << 4).as_u8();

    serialized[7] = (coefficient4 >> 4).as_u8();

    serialized[8] = (coefficient4 >> 12).as_u8();
    serialized[8] |= (coefficient5 << 1).as_u8();

    serialized[9] = (coefficient5 >> 7).as_u8();
    serialized[9] |= (coefficient6 << 6).as_u8();

    serialized[10] = (coefficient6 >> 2).as_u8();

    serialized[11] = (coefficient6 >> 10).as_u8();
    serialized[11] |= (coefficient7 << 3).as_u8();

    serialized[12] = (coefficient7 >> 5).as_u8();
}
...
pub fn deserialize(serialized: &[U8], simd_unit: &mut Coefficients) {
    ...
    let byte3 = serialized[3].as_i32();
    let byte4 = serialized[4].as_i32();
    let byte5 = serialized[5].as_i32();
    let byte6 = serialized[6].as_i32();
    let byte7 = serialized[7].as_i32();
    let byte8 = serialized[8].as_i32();
    let byte9 = serialized[9].as_i32();
    let byte10 = serialized[10].as_i32();
    let byte11 = serialized[11].as_i32();
    let byte12 = serialized[12].as_i32();
