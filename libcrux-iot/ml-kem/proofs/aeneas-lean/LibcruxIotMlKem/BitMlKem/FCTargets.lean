...
private theorem lift_poly_lanes_canonical
...
private theorem lift_fe_mont_mul_pure_eq
    (a c r : Std.I16)
    (h : (r.val * (2 ^ 16 : Int)) % 3329 = (a.val * c.val) % 3329) :
    lift_fe r
      = libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.mul_pure
...
/-! ### L2.9 — `inv_ntt_layer_1_step` (FC).
    ...
    plus the per-output bound `≤ 13312` so the bound is preserved across
    the 8-step chain on disjoint lane pairs. -/
...
/-- Per-lane variant of `inv_ntt_step_fc` for layer composition. Splits
    ...
    3. **Per-output bound `≤ 13312`** at both `i` and `j` — needed to chain
       across steps. `r[i]` is `barrett(vec[j] + vec[i])` with `|·| ≤ 3328`
       (post-barrett); `r[j]` is `montgomery_multiply(vec[j] - vec[i], zeta)`
       with `|·| ≤ 3328 + 1665 = 4993`. Both ≤ 13312 with margin. -/
private theorem inv_ntt_step_pair_fc
    (vec : libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (zeta : Std.I16) (i j : Std.Usize)
    (hi : i.val < 16) (hj : j.val < 16)
    (hne : i.val ≠ j.val)
    ...
    ⦃ ⇓ r => ⌜ lift_chunk r
              ...
              ∧ (∀ k : Nat, k < 16 → k ≠ i.val → k ≠ j.val →
                  (r.elements.val[k]!) = (vec.elements.val[k]!))
              ∧ (r.elements.val[i.val]!).val.natAbs ≤ 13312
              ∧ (r.elements.val[j.val]!).val.natAbs ≤ 13312 ⌝ ⦄ := by
  -- Step 0: vector length facts.
  have h_vec_len : vec.elements.length = 16 :=
    libcrux_iot_ml_kem.Util.PortableVector_elements_length vec
  have h_vec_val_len : vec.elements.val.length = 16 := h_vec_len
  ...
  · -- Untouched-lane preservation: r[k] = vec[k] for k ≠ i, j.
    ...
    rw [List.getElem_set_ne (Ne.symm hki)]
    rw [getElem!_pos vec.elements.val k (by rw [h_vec_val_len]; exact hk)]
  · -- Bound at i: r[i] = o0 (set last) — wait, j may equal i — no, hne i ≠ j.
    -- Actually (set i o0).set j o1 at index i: since i ≠ j, second set doesn't touch i,
    -- so r[i] = (set i o0)[i] = o0. |o0| ≤ 3328 ≤ 13312.
    show ((vec.elements.set i o0).set j o1).val[i.val]!.val.natAbs ≤ 13312
    have h_set_val_eq : ((vec.elements.set i o0).set j o1).val
        = (vec.elements.val.set i.val o0).set j.val o1 := by
      simp [Std.Array.set_val_eq]
    rw [h_set_val_eq]
    ...
    rw [List.getElem_set_self]
    -- now goal: o0.val.natAbs ≤ 13312. h_o0_bd : o0.val.natAbs ≤ 3328.
    omega
  · -- Bound at j: r[j] = o1 (second set wins). |o1| ≤ 3328+1665 = 4993 ≤ 13312.
    show ((vec.elements.set i o0).set j o1).val[j.val]!.val.natAbs ≤ 13312
    have h_set_val_eq : ((vec.elements.set i o0).set j o1).val
        = (vec.elements.val.set i.val o0).set j.val o1 := by
      simp [Std.Array.set_val_eq]
    rw [h_set_val_eq]
    rw [getElem!_pos _ j.val (by simp [List.length_set, h_vec_val_len]; exact hj)]
    rw [List.getElem_set_self]
    -- goal: o1.val.natAbs ≤ 13312. h_o1_bd : o1.val.natAbs ≤ 3328 + 1665.
    omega
...
private theorem polynomial.zeta_eq_ok_fc
  ...
  exact libcrux_iot_ml_kem.Util.array_index_usize_ok_eq
...
@[spec high]
private theorem polynomial.zeta_fc (i : Std.Usize) (hi : i.val < 128) :
    ⦃ ⌜ True ⌝ ⦄
    libcrux_iot_ml_kem.polynomial.zeta i
...
private theorem chunk_at_lift_poly_fc
    (re : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (k : Nat) (hk : k < 16) :
...
private theorem flatten_chunks_eq_lift_poly_fc
    (re : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (chunks : Std.Array (Std.Array hacspec_ml_kem.parameters.FieldElement 16#usize)
                16#usize)
...
private theorem ntt_at_layer_1_step_lemma_fc
    (zeta_i_0 : Std.Usize)
    (re : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
...
private theorem ntt_at_layer_2_step_lemma_fc
    (zeta_i_0 : Std.Usize)
    (re : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (h_pre : ∀ chunk : Nat, chunk < 16 → ∀ ℓ : Nat, ℓ < 16 →
...
private theorem iter_next8_some_eq (i : Std.Usize)
    (h_lt : i.val < (8#usize : Std.Usize).val) :
    ∃ s : Std.Usize, s.val = i.val + 1 ∧
      core_models.iter.range.IteratorRange.next
        core_models.Usize.Insts.Core_modelsIterRangeStep
        ({ start := i, «end» := 8#usize } : core_models.ops.range.Range Std.Usize)
      = .ok (some i,
          ({ start := s, «end» := 8#usize } : core_models.ops.range.Range Std.Usize)) := by
  have hT := libcrux_iot_ml_kem.Util.IteratorRange_next_spec_usize i 8#usize
  ...
  have hex : ∃ v, core_models.iter.range.IteratorRange.next
        core_models.Usize.Insts.Core_modelsIterRangeStep
        ({ start := i, «end» := 8#usize } : core_models.ops.range.Range Std.Usize) = .ok v
      ∧ (∃ s : Std.Usize, s.val = i.val + 1
        ∧ v = (some i,
                ({ start := s, «end» := 8#usize }
                  : core_models.ops.range.Range Std.Usize))) := by
    generalize hx : core_models.iter.range.IteratorRange.next
        core_models.Usize.Insts.Core_modelsIterRangeStep
        ({ start := i, «end» := 8#usize } : core_models.ops.range.Range Std.Usize) = X at hT
    match X, hT with
    | .ok v, hT => exact ⟨v, rfl, by simpa [Std.Do.Triple, Std.Do.WP.wp] using hT⟩
    | .fail _, hT => exact absurd hT (by simp [Std.Do.Triple, Std.Do.WP.wp])
...
private theorem iter_next8_none_eq (i : Std.Usize)
  ...
  | .ok v, hT =>
      have hP : v = ((none : Option Std.Usize),
                    ({ start := i, «end» := 8#usize }
                      : core_models.ops.range.Range Std.Usize)) := by
        simpa [Std.Do.Triple, Std.Do.WP.wp] using hT
      rw [hP]
  | .fail _, hT => exact absurd hT (by simp [Std.Do.Triple, Std.Do.WP.wp])
  | .div, hT => exact absurd hT (by simp [Std.Do.Triple, Std.Do.WP.wp])
...
private theorem ntt_at_layer_7_step_lemma_fc
    (re : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (h_pre : ∀ chunk : Nat, chunk < 16 → ∀ ℓ : Nat, ℓ < 16 →
  ...
  · -- `Some j = k` branch.
    ...
    have h_idx_i : Aeneas.Std.Array.index_usize acc.1.coefficients i_idx
        = .ok scratch1 :=
      libcrux_iot_ml_kem.Util.array_index_usize_ok_eq acc.1.coefficients i_idx
    ...
    have h_s2_spec :=
      libcrux_iot_ml_kem.Equivalence.multiply_by_constant_spec scratch1 (-1600)#i16 h_prod_bnd
    ...
    have h_upd_a : Aeneas.Std.Array.update acc.1.coefficients i_idx t
        = .ok a :=
      libcrux_iot_ml_kem.Util.array_update_ok_eq acc.1.coefficients i_idx t
        (by rw [h_coef_len, h_i_val_arith]; exact hk8_lt_16)
    ...
    have h_imt_j : Aeneas.Std.Array.index_mut_usize a k
      ...
      have h_idx : Aeneas.Std.Array.index_usize a k = .ok (a.val[k.val]!) :=
        libcrux_iot_ml_kem.Util.array_index_usize_ok_eq a k (by rw [h_a_len]; exact hk_lt_16)
      rw [h_idx]
    ...
    have h_imt_i : Aeneas.Std.Array.index_mut_usize a1 i_idx
      ...
      have h_idx : Aeneas.Std.Array.index_usize a1 i_idx = .ok (a1.val[i_idx.val]!) :=
        libcrux_iot_ml_kem.Util.array_index_usize_ok_eq a1 i_idx
...
private theorem iter_next_some_eq_gen (i e : Std.Usize)
    (h_lt : i.val < e.val) :
    ∃ s : Std.Usize, s.val = i.val + 1 ∧
      core_models.iter.range.IteratorRange.next
        core_models.Usize.Insts.Core_modelsIterRangeStep
  ...
  have hT := libcrux_iot_ml_kem.Util.IteratorRange_next_spec_usize i e
    (Q := PostCond.noThrow fun (oi : Option Std.Usize × _) => ⌜
      ∃ s : Std.Usize, s.val = i.val + 1
        ∧ oi = (some i,
                ({ start := s, «end» := e }
  ...
  have hex : ∃ v, core_models.iter.range.IteratorRange.next
        core_models.Usize.Insts.Core_modelsIterRangeStep
        ({ start := i, «end» := e } : core_models.ops.range.Range Std.Usize) = .ok v
      ∧ (∃ s : Std.Usize, s.val = i.val + 1
        ∧ v = (some i,
                ({ start := s, «end» := e }
                  : core_models.ops.range.Range Std.Usize))) := by
    generalize hx : core_models.iter.range.IteratorRange.next
        core_models.Usize.Insts.Core_modelsIterRangeStep
        ({ start := i, «end» := e } : core_models.ops.range.Range Std.Usize) = X at hT
    match X, hT with
    | .ok v, hT => exact ⟨v, rfl, by simpa [Std.Do.Triple, Std.Do.WP.wp] using hT⟩
    | .fail _, hT => exact absurd hT (by simp [Std.Do.Triple, Std.Do.WP.wp])
    | .div, hT => exact absurd hT (by simp [Std.Do.Triple, Std.Do.WP.wp])
...
private theorem iter_next_none_eq_gen (i e : Std.Usize)
    (h_ge : i.val ≥ e.val) :
    core_models.iter.range.IteratorRange.next
      core_models.Usize.Insts.Core_modelsIterRangeStep
      ({ start := i, «end» := e } : core_models.ops.range.Range Std.Usize)
...
private theorem ntt_layer_int_vec_step_fc
  ...
  have h_s2_legacy :=
    libcrux_iot_ml_kem.Equivalence.montgomery_multiply_by_constant_spec scratch1 zeta_r hzeta
  ...
  have h_upd_b : Aeneas.Std.Array.update coefficients b t = .ok c1 :=
    libcrux_iot_ml_kem.Util.array_update_ok_eq coefficients b t
      (by rw [h_coef_len]; exact hb)
  ...
  have h_imt_a : Aeneas.Std.Array.index_mut_usize c1 a = .ok (t, c1.set a) := by
    ...
    have h_idx : Aeneas.Std.Array.index_usize c1 a = .ok (c1.val[a.val]!) :=
      libcrux_iot_ml_kem.Util.array_index_usize_ok_eq c1 a (by rw [h_c1_len]; exact ha)
    rw [h_idx, h_c1_a]; rfl
  ...
  have h_t2_legacy :=
    libcrux_iot_ml_kem.Equivalence.add_spec t scratch2 h_t_s2_add_bnd
  ...
  have h_imt_b : Aeneas.Std.Array.index_mut_usize c2 b = .ok (t, c2.set b) := by
    ...
    have h_idx : Aeneas.Std.Array.index_usize c2 b = .ok (c2.val[b.val]!) :=
      libcrux_iot_ml_kem.Util.array_index_usize_ok_eq c2 b (by rw [h_c2_len]; exact hb)
    rw [h_idx, h_c2_b]; rfl
  ...
  obtain ⟨t4, h_t4_eq, h_t4_lift⟩ :=
    triple_exists_ok_fc (sub_fc t scratch2 h_t_s2_sub_bnd)
  have h_t4_legacy :=
    libcrux_iot_ml_kem.Equivalence.sub_spec t scratch2 h_t_s2_sub_bnd
  ...
  · -- (a) c3[a] = c2[a] (since c3 = c2.set b t4, and a ≠ b).
    ...
    · intro ℓ hℓ1 _
      ...
      have h_s1_len : scratch1.elements.val.length = 16 :=
        libcrux_iot_ml_kem.Util.PortableVector_elements_length scratch1
      have h_s2_len : scratch2.elements.val.length = 16 :=
        libcrux_iot_ml_kem.Util.PortableVector_elements_length scratch2
...
private theorem ntt_at_layer_4_plus_inner_step_lemma_fc
    (re0 : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
...
private theorem outer_acc_a_chunk_eq_re0
...
private theorem outer_acc_b_chunk_eq_re0
    (re0 acc : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (k : Std.Usize) (zeta_i_0 step_vec : Std.Usize)
    (h_undone : ∀ c : Nat, c < 16 →
...
private theorem ntt_at_layer_4_plus_inner_loop_fc
    (re0 : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (scratch : libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (a_offset b_offset step_vec : Std.Usize) (zeta_i1 : Std.Usize)
...
/-- Per-iteration FC step lemma for the outer loop. Dispatches the
    inner loop wrapper. -/
private theorem ntt_at_layer_4_plus_outer_step_lemma_fc
    (re0 : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (zeta_i_0 step_vec i_end : Std.Usize)
    (h_pre : ∀ chunk : Nat, chunk < 16 → ∀ ℓ : Nat, ℓ < 16 →
...
private theorem poly_barrett_reduce_step_lemma_fc
    (self : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
...
private theorem invert_ntt_at_layer_1_step_lemma_fc
    (zeta_i_0 : Std.Usize)
    (re : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (h_pre : ∀ chunk : Nat, chunk < 16 → ∀ ℓ : Nat, ℓ < 16 →
...
/-- Per-iteration FC step lemma for the inverse layer-3 driver. Given a valid
    ...
    the FC equation for chunk `k.val`, leaving chunks `> k.val` unchanged. -/
private theorem invert_ntt_at_layer_3_step_lemma_fc
    (zeta_i_0 : Std.Usize)
    (re : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (h_pre : ∀ chunk : Nat, chunk < 16 → ∀ ℓ : Nat, ℓ < 16 →
...
private theorem subtract_reduce_step_lemma_fc
    (self b_init : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
...
private theorem add_error_reduce_step_lemma_fc
    (self_init error : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (h_self_bnd : ∀ chunk : Nat, chunk < 16 → ∀ ℓ : Nat, ℓ < 16 →
...
private theorem add_standard_error_reduce_step_lemma_fc
    (self_init error : libcrux_iot_ml_kem.polynomial.PolynomialRingElement
            libcrux_iot_ml_kem.vector.portable.vector_type.PortableVector)
    (h_self_bnd : ∀ chunk : Nat, chunk < 16 → ∀ ℓ : Nat, ℓ < 16 →
      ((self_init.coefficients.val[chunk]!).elements.val[ℓ]!).val.natAbs ≤ 32767)
...
private theorem slice_index_range_ok_eq_fc
    {T : Type} (a : Slice T) (r : core_models.ops.range.Range Std.Usize)
...
private theorem poly_reducing_from_i32_array_step_lemma_fc
...
private theorem entry_eq_ok_fc_aux
    (K : Std.Usize)
  ...
  -- Step 1: `core_models.slice.Slice.len matrix` = `.ok matrix.len`.
