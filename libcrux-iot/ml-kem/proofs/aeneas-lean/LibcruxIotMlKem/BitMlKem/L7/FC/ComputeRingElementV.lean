...
import LibcruxIotMlKem.BitMlKem.L7.Impl.ComputeRingElementV
...
import LibcruxIotMlKem.BitMlKem.L7.Correctness.ComputeRingElementV
...
theorem compute_ring_element_v_fc
  ...
  · unfold libcrux_iot_ml_kem.matrix.compute_ring_element_v
    ...
    rw [show (core_models.slice.Slice.chunks_exact public_key (384#usize : Std.Usize))
          = .ok { cs := 384#usize, elements := public_key } from rfl]
    simp only [Aeneas.Std.bind_tc_ok]
    rw [show (core_models.slice.iter.ChunksExact.Insts.Core_modelsIterTraitsIteratorIteratorSharedASlice.enumerate
