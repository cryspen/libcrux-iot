...
fn test_invalid_modulus(p: &str) {
    ...
    for line in reader.lines() {
        ...
        match p {
            ...
            "512" => assert!(!libcrux_iot_ml_kem::mlkem512::validate_public_key(
                &pk.try_into().unwrap()
            )),
            #[cfg(feature = "mlkem768")]
            "768" => assert!(!libcrux_iot_ml_kem::mlkem768::validate_public_key(
                &pk.try_into().unwrap()
            )),
            #[cfg(feature = "mlkem1024")]
            "1024" => assert!(!libcrux_iot_ml_kem::mlkem1024::validate_public_key(
                &pk.try_into().unwrap()
            )),
            _ => unreachable!(),
        };
