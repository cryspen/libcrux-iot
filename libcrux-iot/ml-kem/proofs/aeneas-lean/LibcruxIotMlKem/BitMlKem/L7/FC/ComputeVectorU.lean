...
import LibcruxIotMlKem.BitMlKem.L7.Impl.ComputeVectorU
...
private theorem multiply_vectors_eq_mcolL {K : Std.Usize}
  ...
  have h_triple : ⦃ ⌜ True ⌝ ⦄
      hacspec_ml_kem.matrix.multiply_vectors_loop
        ({ start := 0#usize, «end» := K } : core_models.ops.range.Range Std.Usize)
        col vec
    ...
    apply Std.Do.Triple.of_entails_right _
      (libcrux_iot_ml_kem.Util.loop_range_spec_usize
        (fun p : core_models.ops.range.Range Std.Usize × Poly256L =>
...
private theorem transpose_inner_eq {K : Std.Usize}
    ...
    hacspec_ml_kem.matrix.transpose.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayArrayFieldElement256RANK.call
        (RANK := K) (m) j
      = .ok ⟨(List.range K.val).map (fun i' => (m.val[i']!).val[j.val]!),
             by simp [List.length_map, List.length_range]⟩ := by
  unfold hacspec_ml_kem.matrix.transpose.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayArrayFieldElement256RANK.call
  show hacspec_ml_kem.parameters.createi K
      (hacspec_ml_kem.matrix.transpose.closure.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayFieldElement256 K)
      (m, j) = _
  unfold hacspec_ml_kem.parameters.createi
  have hpure : ∀ i' : Nat, i' < K.val →
      (hacspec_ml_kem.matrix.transpose.closure.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayFieldElement256 K).FnMutInst.call_mut
        (m, j) ⟨BitVec.ofNat _ i'⟩
        = .ok ((m.val[i']!).val[j.val]!, (m, j)) := by
    intro i' hi'
    show hacspec_ml_kem.matrix.transpose.closure.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeArrayFieldElement256.call_mut
        (m, j) ⟨BitVec.ofNat _ i'⟩
      = .ok ((m.val[i']!).val[j.val]!, (m, j))
    unfold hacspec_ml_kem.matrix.transpose.closure.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeArrayFieldElement256.call_mut
    unfold hacspec_ml_kem.matrix.transpose.closure.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayFieldElement256.call
  ...
  have h := libcrux_iot_ml_kem.Util.createi_pure_eq
    ...
    (N := K)
    (inst := hacspec_ml_kem.matrix.transpose.closure.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayFieldElement256 K)
    (c := (m, j))
...
private theorem transpose_row_eq {K : Std.Usize}
  ...
  have hpure : ∀ j : Nat, j < K.val →
      (hacspec_ml_kem.matrix.transpose.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayArrayFieldElement256RANK K).FnMutInst.call_mut
        (m) ⟨BitVec.ofNat _ j⟩
        = .ok (f j, m) := by
    intro j hj
    show hacspec_ml_kem.matrix.transpose.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeArrayArrayFieldElement256RANK.call_mut
        (m) ⟨BitVec.ofNat _ j⟩ = .ok (f j, m)
    unfold hacspec_ml_kem.matrix.transpose.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeArrayArrayFieldElement256RANK.call_mut
    ...
    show (do let a ← hacspec_ml_kem.matrix.transpose.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayArrayFieldElement256RANK.call
  ...
  have h := libcrux_iot_ml_kem.Util.from_fn_pure_eq
    ...
    (N := K)
    (inst := (hacspec_ml_kem.matrix.transpose.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayArrayFieldElement256RANK K).FnMutInst)
    (c := m)
    (f := f)
    hpure
...
private theorem createi_stage_eq {K : Std.Usize} {T F : Type} [Inhabited T]
    (inst : core_models.ops.function.Fn F Std.Usize T) (c : F)
    (g : Nat → Result T)
    (hcall : ∀ k : Nat, k < K.val →
...
private theorem compute_vector_u_hacspec_eq {K : Std.Usize}
  ...
  have h_inv_stage :
      hacspec_ml_kem.parameters.createi K
          (hacspec_ml_kem.matrix.compute_vector_u.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayFieldElement256 K) P
        = .ok ⟨(List.range K.val).map (fun k => resGet (g_inv k)),
               by simp [List.length_map, List.length_range]⟩ := by
    apply createi_stage_eq _ _ g_inv _ hg_inv_ok
    intro k hk
    show hacspec_ml_kem.matrix.compute_vector_u.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeArrayFieldElement256.call_mut
        P ⟨BitVec.ofNat _ k⟩ = (do let a ← g_inv k; .ok (a, P))
    unfold hacspec_ml_kem.matrix.compute_vector_u.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeArrayFieldElement256.call_mut
    unfold hacspec_ml_kem.matrix.compute_vector_u.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeArrayFieldElement256.call
    have hkv : (⟨BitVec.ofNat _ k⟩ : Std.Usize).val = k := ofNat_toNat_eq k hk
...
theorem compute_vector_u_fc
  ...
  -- massert lengths.
  have h_len_r : core_models.slice.Slice.len r_as_ntt = .ok K := by
    unfold core_models.slice.Slice.len
    apply congrArg Result.ok
    apply Std.UScalar.eq_of_val_eq
    show r_as_ntt.val.length = K.val
    exact h_r_len
  have h_len_e : core_models.slice.Slice.len error_1 = .ok K := by
    unfold core_models.slice.Slice.len
    apply congrArg Result.ok
    apply Std.UScalar.eq_of_val_eq
