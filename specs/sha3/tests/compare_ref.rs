/// Compare our SHA3 implementation against the reference libcrux-iot-sha3 crate.

#[test]
fn sha3_256_vs_reference() {
    let mut ref_digest = [0u8; 32];
    libcrux_iot_sha3::sha256_ema(&mut ref_digest, b"");
...
fn shake128_abc_vs_reference() {
    ...
    libcrux_iot_sha3::shake128_ema(&mut ref_out, b"abc");
...
fn shake256_abc_vs_reference() {
    let mut ref_out = [0u8; 32];
    libcrux_iot_sha3::shake256_ema(&mut ref_out, b"abc");
