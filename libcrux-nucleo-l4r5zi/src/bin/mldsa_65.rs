...
fn bench_sign<L: EventLogger>(_l: &mut L, state: &MLDSABenchState) -> Result<(), String> {
    let _signature = ml_dsa_65::sign(&state.keypair.signing_key, &state.message, b"", state.signing_randomness);
    Ok(())
}

fn bench_verify<L: EventLogger>(_l: &mut L, state: &MLDSABenchState) -> Result<(), String> {
    let _ = ml_dsa_65::verify(&state.keypair.verification_key, &state.message, b"", &state.signature);
...
fn main() -> ! {
    ...
    let state = MLDSABenchState{ randomness_gen, keypair, signing_randomness, message, signature};
