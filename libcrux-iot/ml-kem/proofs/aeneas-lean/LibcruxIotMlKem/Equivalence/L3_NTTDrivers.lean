/-
  ...
  `ntt_layer_1_step` field reduces (via `@[reducible]`) to
  `…Operations.ntt_layer_1_step`, which is itself a thin wrapper for
  `vector.portable.ntt.ntt_layer_1_step` — L2.2 fires directly.

  See `Plan.lean § L3.1` (lines 1458-1514) for F*-port reference.
-/
