...
import LibcruxIotMlKem.BitMlKem.L7.Correctness.Bridges
...
/-! ## B — `invert_ntt_montgomery_pure` is `scaleZ`-equivariant.
    ...
    **Canonical precondition (required).** `SpecPure.FieldElement.sub_pure` is
...
private theorem chunk_inv_ntt_step_lane
  ...
  · rw [if_pos hℓj]
    subst hℓj
    have hbnd : j.val < (a.set i (libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.add_pure
...
private theorem layer4_partner_lt
    (cc step : Nat) (hcc : cc < 16) (hstep : 0 < step) (hdvd : (2 * step) ∣ 16)
...
private theorem invert_ntt_layer_4_plus_canon
    (p : Std.Array hacspec_ml_kem.parameters.FieldElement 256#usize)
    (zeta_i layer : Std.Usize) (hp : CanonArr p) :
...
theorem invert_ntt_montgomery_pure_scaleZ (c : ZMod 3329)
    (p : Std.Array hacspec_ml_kem.parameters.FieldElement 256#usize)
    (hp : ∀ j : Nat, j < 256 →
      libcrux_iot_ml_kem.BitMlKem.SpecPure.Canonical (p.val[j]!)) :
...
private theorem reduce_polynomial_eq_ok
  ...
  set f : Nat → hacspec_ml_kem.parameters.FieldElement :=
    fun k => libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.mul_pure
              (a.val[k]!) ⟨3303#u16⟩ with hf_def
  have hpure : ∀ k : Nat, k < (256#usize : Std.Usize).val →
      (hacspec_ml_kem.invert_ntt.reduce_polynomial.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeFieldElement
        : core_models.ops.function.Fn _ _ _).FnMutInst.call_mut
            a ⟨BitVec.ofNat _ k⟩
        = .ok (f k, a) := by
    intro k hk
    have hk' : k < 256 := hk
    show hacspec_ml_kem.invert_ntt.reduce_polynomial.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeFieldElement.call_mut
        a ⟨BitVec.ofNat _ k⟩ = .ok (f k, a)
    unfold hacspec_ml_kem.invert_ntt.reduce_polynomial.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeFieldElement.call_mut
    unfold hacspec_ml_kem.invert_ntt.reduce_polynomial.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeFieldElement.call
    ...
    have h_a_idx :
        Std.Array.index_usize a (⟨BitVec.ofNat _ k⟩ : Std.Usize)
          = .ok (a.val[k]!) := by
      have := libcrux_iot_ml_kem.Util.array_index_usize_ok_eq a
    ...
    have h_inv : hacspec_ml_kem.invert_ntt.INVERSE_OF_128
      ...
      rfl
    have h_mul :=
      libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.mul_eq_ok
  ...
  exact libcrux_iot_ml_kem.Util.createi_pure_eq 256#usize
    hacspec_ml_kem.invert_ntt.reduce_polynomial.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeFieldElement
    a f hpure
...
private theorem ntt_inverse_reduce_eq
  ...
  have hL_lane : ∀ j : Nat, j < 256 →
      L.val[j]! = libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.mul_pure
                    (q.val[j]!) ⟨3303#u16⟩ := by
    intro j hj
...
/-! ### C2 Phase 1: flat per-lane reduction of `ntt_inverse_layer_n`.
...
private theorem inv_butterfly_eq (z a b : hacspec_ml_kem.parameters.FieldElement)
    ...
    = .ok (libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.add_pure a b,
           libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.mul_pure z
             (libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.sub_pure b a)) := by
  unfold hacspec_ml_kem.invert_ntt.inv_butterfly
  rw [libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.add_eq_ok a b]
  simp only [bind_tc_ok]
  rw [libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.sub_eq_ok b a hb ha]
  simp only [bind_tc_ok]
  rw [libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.mul_eq_ok z _]
  simp only [bind_tc_ok]
...
private theorem layer_n_at_eq
  ...
  · rw [if_pos (by rw [hdec]; exact hbr : idx < len)]
    ...
    rw [if_pos hbr']
    rw [libcrux_iot_ml_kem.Util.slice_index_usize_ok_eq s grp (by rw [hgrpv, hi1v]; exact hsg)]
    simp only [bind_tc_ok]
    rw [libcrux_iot_ml_kem.Util.array_index_usize_ok_eq p i
    ...
    rw [libcrux_iot_ml_kem.Util.array_index_usize_ok_eq p i2
...
private theorem ntt_inverse_layer_n_eq_ok
  ...
  have hpure : ∀ k : Nat, k < (256#usize : Std.Usize).val →
      (hacspec_ml_kem.invert_ntt.ntt_inverse_layer_n.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeFieldElement
        256#usize : core_models.ops.function.Fn _ _ _).FnMutInst.call_mut
            (p, len, s) ⟨BitVec.ofNat _ k⟩
        = .ok (f k, (p, len, s)) := by
    intro k hk
    have hk' : k < 256 := hk
    show hacspec_ml_kem.invert_ntt.ntt_inverse_layer_n.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeFieldElement.call_mut
        (p, len, s) ⟨BitVec.ofNat _ k⟩ = .ok (f k, (p, len, s))
    unfold hacspec_ml_kem.invert_ntt.ntt_inverse_layer_n.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeFieldElement.call_mut
    unfold hacspec_ml_kem.invert_ntt.ntt_inverse_layer_n.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeFieldElement.call
    have hk_us : (⟨BitVec.ofNat _ k⟩ : Std.Usize).val = k := by
      show (BitVec.ofNat _ k).toNat = k
      apply Nat.mod_eq_of_lt
  ...
  exact libcrux_iot_ml_kem.Util.createi_pure_eq 256#usize
    (hacspec_ml_kem.invert_ntt.ntt_inverse_layer_n.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeFieldElement 256#usize)
    (p, len, s) f hpure

/-! ### C2 Phase 1c: `ntt_inverse_layer` (zetas createi + range-slice) reduction.
...
private theorem ntt_inverse_layer_zetas_eq_ok (groups : Std.Usize)
    ...
    (hacspec_ml_kem.parameters.createi 128#usize
      hacspec_ml_kem.invert_ntt.ntt_inverse_layer.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeFieldElement
      groups)
  ...
  have hpure : ∀ g : Nat, g < (128#usize : Std.Usize).val →
    ...
    intro g hg
    have hg' : g < 128 := hg
    show hacspec_ml_kem.invert_ntt.ntt_inverse_layer.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeFieldElement.call_mut
        groups ⟨BitVec.ofNat _ g⟩ = .ok (f g, groups)
    unfold hacspec_ml_kem.invert_ntt.ntt_inverse_layer.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeFieldElement.call_mut
    unfold hacspec_ml_kem.invert_ntt.ntt_inverse_layer.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeFieldElement.call
    ...
    · have hbr' : g < groups.val := by
      ...
      rw [libcrux_iot_ml_kem.Util.array_index_usize_ok_eq zetasArr i3 hi3lt]
  ...
  have h_from_fn :=
    libcrux_iot_ml_kem.Util.from_fn_pure_eq
      ...
      (N := 128#usize)
      (inst := hacspec_ml_kem.invert_ntt.ntt_inverse_layer.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeFieldElement)
      (c := groups)
      (f := f)
      hpure
  unfold hacspec_ml_kem.parameters.createi
  show core_models.array.from_fn 128#usize _ groups = _
  exact h_from_fn
...
private theorem slice0_getElem_at {α} [Inhabited α]
    (l : List α) (b : Nat) (h_le_b : b ≤ l.length) (k : Nat) (hk : k < b) :
...
/-! ### C2 Phase 2: per-layer flat↔chunk match.
...
/-! ### C2 Phase 2 (cont.): per-layer match `ntt_inverse_layer ≡ spec layer`. -/
...
private theorem spec_inv_layer_1_lane
  ...
  · rw [if_neg (by rw [hmod4]; exact hbr), if_neg hbr]
    ...
    have hzsel : (if (i % 16) / 4 = 0 then Spec.zeta_at (128 - 4 * (i / 16) - 1)
      ...
      interval_cases h : ((i % 16) / 4) <;> simp <;> congr 1 <;> omega
    rw [hzsel]
...
private theorem flat_layer_canon
  ...
  · rw [if_pos hbr]; exact libcrux_iot_ml_kem.BitMlKem.SpecPure.Canonical_add_pure _ _
  · rw [if_neg hbr]; exact libcrux_iot_ml_kem.BitMlKem.SpecPure.Canonical_mul_pure _ _
...
private theorem spec_inv_layer_2_lane
  ...
  · rw [if_neg (by rw [hmod8]; exact hbr), if_neg hbr]
    ...
    have hzsel : (if (i % 16) / 8 = 0 then Spec.zeta_at (64 - 2 * (i / 16) - 1)
      ...
      interval_cases h : ((i % 16) / 8) <;> simp <;> congr 1 <;> omega
    rw [hzsel]
...
private theorem spec_inv_layer_4_plus_lane
  ...
  have hdecf : (i % (2 * len) < len) ↔ ((i / 16) % (2 * step) < step) := by
    rw [hmce, hlen]
    constructor
    · intro h; by_contra hco; push_neg at hco
...
private theorem sub_polynomials_eq_ok
    ...
    hacspec_ml_kem.matrix.sub_polynomials a c
      = .ok ⟨(List.range 256).map (fun k =>
             ...
             by simp [List.length_map, List.length_range]⟩ := by
  set f : Nat → hacspec_ml_kem.parameters.FieldElement :=
    fun k => libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.sub_pure
              (a.val[k]!) (c.val[k]!) with hf_def
  have hpure : ∀ k : Nat, k < (256#usize : Std.Usize).val →
      (hacspec_ml_kem.matrix.sub_polynomials.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeFieldElement
        : core_models.ops.function.Fn _ _ _).FnMutInst.call_mut
            (a, c) ⟨BitVec.ofNat _ k⟩
        = .ok (f k, (a, c)) := by
    intro k hk
    have hk' : k < 256 := hk
    show hacspec_ml_kem.matrix.sub_polynomials.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeFieldElement.call_mut
        (a, c) ⟨BitVec.ofNat _ k⟩ = .ok (f k, (a, c))
    unfold hacspec_ml_kem.matrix.sub_polynomials.closure.Insts.Core_modelsOpsFunctionFnMutTupleUsizeFieldElement.call_mut
    unfold hacspec_ml_kem.matrix.sub_polynomials.closure.Insts.Core_modelsOpsFunctionFnTupleUsizeFieldElement.call
    have hk_us : (⟨BitVec.ofNat _ k⟩ : Std.Usize).val = k := by
      show (BitVec.ofNat _ k).toNat = k
      apply Nat.mod_eq_of_lt
      have : k < 2^System.Platform.numBits := by
    ...
    have h_c_idx :
        Std.Array.index_usize c (⟨BitVec.ofNat _ k⟩ : Std.Usize)
          = .ok (c.val[k]!) := by
      have := libcrux_iot_ml_kem.Util.array_index_usize_ok_eq c
                  (⟨BitVec.ofNat _ k⟩ : Std.Usize) hc_len
      rw [hk_us] at this; exact this
    have h_sub :=
      libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.sub_eq_ok
...
theorem sub_polynomials_scaleZ_eq
    ...
    (ha : ∀ j : Nat, j < 256 →
      libcrux_iot_ml_kem.BitMlKem.SpecPure.Canonical (a.val[j]!)) :
  ...
  set L : Std.Array hacspec_ml_kem.parameters.FieldElement 256#usize :=
    ⟨(List.range 256).map (fun k =>
     ...
     by simp [List.length_map, List.length_range]⟩ with hL_def
  have hL_lane : ∀ j : Nat, j < 256 →
      L.val[j]! = libcrux_iot_ml_kem.BitMlKem.SpecPure.FieldElement.sub_pure
  ...
  · -- L lanes canonical
    intro j hj
    rw [hL_lane j hj]
    exact libcrux_iot_ml_kem.BitMlKem.SpecPure.Canonical_sub_pure _ _ (ha j hj) (hc j hj)
  · -- subtract_reduce_pure lanes canonical
    ...
    exact libcrux_iot_ml_kem.BitMlKem.SpecPure.Canonical_sub_pure _ _ (ha j hj)
