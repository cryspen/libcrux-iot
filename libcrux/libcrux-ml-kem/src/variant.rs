...
pub(crate) trait Variant {
    ...
    fn entropy_preprocess<const K: usize>(randomness: &mut [u8]);
...
impl Variant for Kyber {
    ...
    fn kdf<const K: usize, const CIPHERTEXT_SIZE: usize>(
        ...
        PortableHash::PRF::<32>(&kdf_input, shared_secret)
    }

    #[inline(always)]
    fn entropy_preprocess<const K: usize>(randomness: &mut [u8]) {
        let input = randomness.clone();
        PortableHash::H(&input, randomness)
    }
...
impl Variant for MlKem {
    ...
    fn entropy_preprocess<const K: usize>(_randomness: &mut [u8]) {}
