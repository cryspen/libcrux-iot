/-
  ...
  (Plan.lean §493-495 verdict — "Refactor before lift").
  ...
  - `createi_pure_eq` + `createi_pure_spec`
    ...
    `core_models.ops.function.Fn` instance) is the hax extraction of
    `core::array::from_fn` over a *shared* closure.

  - `from_fn_pure_eq` + `from_fn_pure_spec`
    (`LibcruxIotSha3/Sponge/XorBlockSpec.lean:103,137`) —
    the *direct* `FnMut` variant. `core_models.array.from_fn N inst c`
    (where `inst` is `core_models.ops.function.FnMut`) is the hax
  ...
  - The hax extraction of `core::array::from_fn` over a *`FnMut`*-bounded
    ...
    goes directly through `core_models.array.from_fn` and
...
/-! ## `FnMut`-direct variant: `core_models.array.from_fn N inst c`

Analogous to `createi_*` but takes a `core_models.ops.function.FnMut`
instance directly (no `Fn` wrapper). Required when the hax extraction
calls `core_models.array.from_fn` directly with the `FnMut` instance of
