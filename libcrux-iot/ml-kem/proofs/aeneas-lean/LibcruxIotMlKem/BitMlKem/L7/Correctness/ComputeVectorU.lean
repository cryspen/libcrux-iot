/-
  # `BitMlKem/L7/Correctness/ComputeVectorU.lean` — L7.2 `D''` tail bridge.
...
theorem add_polynomials_scaleZ_eq
    (b e : Std.Array hacspec_ml_kem.parameters.FieldElement 256#usize)
    (hb : ∀ j : Nat, j < 256 →
      libcrux_iot_ml_kem.BitMlKem.SpecPure.Canonical (b.val[j]!)) :
...
private theorem triple_exists_ok_fc' {α : Type} {x : Result α} {P : α → Prop}
    ...
    ∃ v, x = .ok v ∧ P v := by
  match hx : x with
  | .ok v => exact ⟨v, rfl, (by subst hx; simpa [Std.Do.Triple, Std.Do.WP.wp] using h)⟩
  | .fail _ => exact absurd h (by simp [Std.Do.Triple, Std.Do.WP.wp])
