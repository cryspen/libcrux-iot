/-
  ...
  This module lifts `modq_eq` (previously defined inline in `Plan.lean`)
  into a standalone file together with its standard algebraic lemma
  surface (reflexivity, symmetry, transitivity, additive / subtractive
  / constant-multiplicative compatibility, and a bridge to `ZMod q`).

  Every L0–L9 Triple postcondition that asserts `a ≡ b (mod q)` (where
  `q = 3329` for ML-KEM) uses this single named predicate. See
  `Plan.lean` lines 222–250 for the rationale behind the
  subtraction-mod spelling `(a - b) % q = 0` (vs `a % q = b % q`):
  it composes additively without side conditions and matches the way
  `lemma_mod_add_distr` is invoked in the upstream F* proofs.

  Used by every L0–L3 ML-KEM Triple plus all ML-DSA arithmetic; lift
...
/-- Canonical modular-equality predicate: `a ≡ b (mod q)` in the
    ...
    discharge directly via `decide` / `scalar_tac` on concrete
    keystones (see `Plan.lean § "modq_eq"` for the rationale). -/
